          ...
          <div className="min-w-screen md:min-w-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 p-4">
              {movies.map((movie, index) => (
                <div key={index} className="border-2 border-red-500 h-[250px] w-full relative" style={{ background: `url(${movie.posterUrl})`, backgroundSize: 'cover' }}>
                  <div className="bg-white h-10 w-full absolute bottom-0 flex items-center justify-between px-2">
                    <h1 className="text-black font-bold text-center truncate">{movie.title}</h1>
                    <button onClick={() => handleDelete(movie.title)} className="text-red-500 hover:text-red-700">
